...
if (API_KEYS.length === 0) {
    console.error("❌ No API Keys found in environment! Please ensure GEMINI_API_KEY or VITE_GEMINI_API_KEY is set.");
...
async function generateUnitList(grade: string, subject: string, language: string) {
    ...
    try {
        const response = await ai.models.generateContent({
            model: "gemini-3-flash-preview", // Use Flash Lite for lists (cheap/fast)
            contents: prompt,
            config: {
